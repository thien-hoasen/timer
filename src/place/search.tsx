...
export function PlaceSearch(props: {
  ...
  isSearchTimezone: boolean
  setIsSearchTimezone: (isSearchTimezone: boolean) => void
}): ReactElement {
  const { timezones, setTimezones, isSearchTimezone, setIsSearchTimezone } = props
  ...
  return (
    <Dialog.Root open={isSearchTimezone} onOpenChange={setIsSearchTimezone}>
      <Dialog.Trigger>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup
          className={twJoin(
            'fixed bottom-0 left-0 w-full h-11/12 flex justify-center',
        ...
        >
          <div
            className={twJoin(
              'p-16 rounded-t-16 bg-gray-1 w-full max-w-[1000px] shadow-6',
